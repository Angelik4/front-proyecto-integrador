...
const FormAddSalas = ({ isOpen, onRequestClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Envía las rutas de las imágenes al servidor
      const imagenes = paths.map((path, index) => ({
        nombre: `imagen_${index}`,
        imagen: path,
        idSala: idSala
      }));
  
      console.log("Datos de las imágenes a enviar:", imagenes);
  
      // Envía la solicitud al servidor después de que todas las imágenes se hayan subido correctamente
      const responseImagenes = await sendRequest("POST", "http://localhost:8081/imagen/registrar", imagenes);
